...
export default function DestinationPlanner() {
  ...
  const safeMustVisit = plan && Array.isArray(plan.must_visit) ? plan.must_visit : [];
      ...
      {plan && (
        <div className="bg-slate-800/80 backdrop-blur-md p-6 rounded-2xl border border-slate-700/60 shadow-xl space-y-6">
          <div className="flex items-center gap-2 text-2xl font-black text-teal-300 border-b border-slate-700/60 pb-3">
            ...
            <h3>Itinerary Guide for {plan.destination || 'Destination'}</h3>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-700/50 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-teal-300 font-bold text-base">{plan.estimated_budget_per_day || '$80 - $150 USD'}</p>
              </div>
            </div>
            ...
            <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-700/50 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-amber-300 font-bold text-base">{plan.best_time_to_visit || 'Spring or Autumn'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/90 p-4 rounded-xl border-l-4 border-teal-400 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs md:text-sm text-slate-300 leading-relaxed">{plan.local_tips || 'Explore early in the morning to avoid crowds and use public transit passes.'}</p>
            </div>
          </div>
        </div>
      )}
